...
function MLOS_SortingRules:updateSortingRule(modId, data)
	if utils:tableIsEmpty(self.data) then
		self.data = self:readSortingRules()
	end
...
---@param fileName string|nil optional
function MLOS_SortingRules:saveSortingRules(srData, fileName)
	local rulesFile = core.MLOS_ROOT .. "/" .. (fileName or RULES_NAME)
	print("MLOS - Saving sorting rules: ", rulesFile)
	core:saveTxtFile(rulesFile, srData or self.data or {}, function(...) return self:getSRDataText(...) end)
end
...
function MLOS_SortingRules:readSortingRules(fileName)
	local rulesName = fileName or RULES_NAME
	local rulesFile = core.MLOS_ROOT .. "/" .. rulesName
	local result = nil
	print("MLOS - Reading sorting rules: ", rulesFile)
	-- support auto moving sorting rules to the separate mod folder. will be removed after some game versions (initially added for b42.19.0)
	if fileName == RULES_NAME and cacheFileExists(rulesFile) == false then
		if cacheFileExists(rulesName) == false then self:saveSortingRules(); return self.data end
		result = core:readTxtFile(rulesName, function(addTo, line) return core:getDataFromString(line, addTo) end )
		self:saveSortingRules(result, rulesFile)
		self:doRulesBackup()
		return result  or {}
	end

	result = core:readTxtFile(rulesFile, function(addTo, line) return core:getDataFromString(line, addTo) end)
	self:doRulesBackup()
	return result or {}
